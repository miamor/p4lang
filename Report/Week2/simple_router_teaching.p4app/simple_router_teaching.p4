...
control HashtableUpdate(in register<bit<32>> hashtable,
    ...
    action update_hashtable() {
        ...
        Read the previous contents of that address, 
        add the packet length to the previous bytecount, 
        update the register address and keep a copy of the value in the metadata.
        */
...
control HHD(inout headers hdr,
    ...
    action calculate_age() {
        ...
        */
        last_reset_time.read(meta.hhd.filter_age, 32w0); // read last reset
        meta.hhd.filter_age = standard_metadata.ingress_global_timestamp - meta.hhd.filter_age; // update meta.hhd.filter_age as how long from the last reset
    }
   
    action set_threshold(bit<32> threshold) {
        /* TODO
        Copy the threshold to metamhhd.threshold
        */
    ...
    table threshold_table {
        key = {
            meta.hhd.filter_age : ternary;  // not sure this match kind
        }
